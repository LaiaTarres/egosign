...
class WandBProgressBarWrapper(BaseProgressBar):
    ...
    def _log_to_wandb(self, stats, tag=None, step=None):
        ...
        for key in stats.keys() - {"num_updates", "preds", "targets"}:
            if isinstance(stats[key], AverageMeter):
                ...
                #We add this for the confusion matrix.
                #check that the sum of all the rows == sum of all the column
                ...
                wandb.log({prefix + key: wandb.Image(plt)}, step=step)

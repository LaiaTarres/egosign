...
class AverageMeter(Meter):
    ...
    def update(self, val, n=1): #value and weight that this contributes
        if val is not None:
            self.val = val
            if n > 0:
...
'''
Laia: I have added this
'''
import numpy as np
from typing import List

class AppendMeter(Meter):
    """Appends and stores current value"""
    ...
    def state_dict(self):
        return {"values": [v.tolist() for v in self.values]}

    def load_state_dict(self, state_dict):
        self.values = [np.array(v) for v in state_dict["values"]]
    ...
    @property
    def smoothed_value(self) -> float:
        return self.values[0] #Let's just return the first value.
    
#class ConfusionMatrixMeter(Meter):
#    """
#    A meter to store and report a confusion matrix.
#    """
#
#    def __init__(self, fn, num_classes):
...
#
#        Args:
#            preds (list or numpy.ndarray): Predicted labels (1D array).
#            targets (list or numpy.ndarray): True labels (1D array).
#        """
#        preds = np.array(preds)
#        targets = np.array(targets)
#        assert preds.shape == targets.shape, f"Predictions and targets shapes mismatch: {preds.shape} vs {targets.shape}"
#        np.add.at(self.matrix, (targets, preds), 1)  # Efficiently updates the matrix
#
#    def get_value(self):
...
#
#    def state_dict(self):
#        return {"matrix": self.matrix.tolist(), "num_classes": self.num_classes}
#
#    def load_state_dict(self, state_dict):
#        self.matrix = np.array(state_dict["matrix"])

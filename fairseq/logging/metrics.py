...
'''
I have had to add this to add the confusion matrix. If I can think of a better way to do this, I will update it later.
'''
def log_array(
    key: str,
    value: np.array,
    ...
    for agg in get_active_aggregators():
        if key not in agg:
            agg.add_meter(key, AppendMeter(), priority) #This we ned not to be an AverageMeter

        agg[key].update(value)
        
def log_derived_matrix(key: str, fn: Callable[[MetersDict], np.ndarray], priority: int = 20):
    """Log a matix derived from other meters.
    ...
    for agg in get_active_aggregators():
        if key not in agg:
            agg.add_meter(key, MetersDict._DerivedMeter(fn), priority) #This we should change to the aggregator that we have.

...
from fairseq.data.sign_language import SignFeatsType_TD
...
class SLTopicDetectionTransformerConfig(FairseqDataclass):
    ...
    feats_type: ChoiceEnum([x.name for x in SignFeatsType_TD]) = II("task.feats_type")
...
class SLTopicDetectionTransformerEncoder_CLS(FairseqEncoder):
    ...
    def __init__(self, cfg, encoder_embed_tokens=None):
        ...
        self.register_parameter(
            ...
            param=Parameter(
                torch.empty((cfg.encoder_embed_dim if cfg.feats_type not in ['spot_align_albert', 'text_albert', 'mouthings_albert'] else
                                get_num_feats(
                                    SignFeatsType_TD[cfg.feats_type],
        ...
        self.feats_type = SignFeatsType_TD[cfg.feats_type]
        ...
        if cfg.subsample_input:
            self.subsample = Conv1dSubsampler(
                get_num_feats(
                    ...
                    cfg.body_parts.split(','),
                    cfg.feat_dims.split(',')
                ) if self.feats_type not in [SignFeatsType_TD.text, SignFeatsType_TD.spot_align, SignFeatsType_TD.mouthings] else cfg.encoder_embed_dim,
                cfg.conv_channels,
    ...
    def _forward(self, src_tokens, src_lengths, return_all_hiddens=False):
        ...
        if self.feats_type in [SignFeatsType_TD.text, SignFeatsType_TD.spot_align, SignFeatsType_TD.mouthings]:
        ...
        encoder_padding_mask = lengths_to_padding_mask(input_lengths) #num_inputs, input_length
        positions = self.embed_positions(encoder_padding_mask).transpose(0, 1) #torch.Size([637, 3, 256])
        x += positions

...
class SLTopicDetectionLSTMModel(BaseFairseqModel):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        '''
        #In case we are working with keypoints, this needs to be adjusted
        if self.cfg.feats_type == "mediapipe_keypoints": #We need to reshape this, to be B x T
            src_tokens = src_tokens.view(src_tokens.shape[0], src_tokens.shape[1], -1)# B x T x C 
...
class SLTopicDetectionLSTMEncoder(LSTMEncoder):
    ...
    def __init__(
        ...
        self.lstm = LSTM(
            input_size=(cfg.encoder_input_embed_size
                        if SignFeatsType_TD[cfg.feats_type] in [SignFeatsType_TD.text, SignFeatsType_TD.spot_align, SignFeatsType_TD.mouthings] else
                        get_num_feats(
    ...
    def forward(self, src_tokens, src_lengths):
        if self.num_updates < self.encoder_freezing_updates:
            with torch.no_grad():
                x = self._forward(
                        src_tokens, src_lengths, enforce_sorted='albert'!=SignFeatsType_TD[self.cfg.feats_type].name[-6:]
                )
        else:
            x = self._forward(
                src_tokens, src_lengths, enforce_sorted='albert'!=SignFeatsType_TD[self.cfg.feats_type].name[-6:]

...
class PerceiverModel(FairseqEncoderDecoderModel):
    ...
    def _forward(
        ...
        if self.input_preprocessor is not None:
            ...
            # We need to adapt to the correct shape:
            if self.cfg.feats_type == "mediapipe_keypoints": #We need to reshape this, to be B x T
                inputs = inputs.view(inputs.shape[0], inputs.shape[1], -1)# B x T x C 
        
            batch_size, seq_length, _ = inputs.size()
            modality_sizes = None
            inputs_without_pos = None

        if self.cfg.feats_type in ['i3d', 'keypoints', 'rotational', 'mediapipe_keypoints'] and inputs.size()[-1] != self.cfg.d_model:
            raise ValueError(

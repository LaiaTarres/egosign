...
class SignFeatsDataset(FairseqDataset):
    def __init__(
        ...
        shuffle: bool = True,
        bodyparts: Optional[List[str]] = None, # We add this to be able to select what we want to keep.
    ...
    def from_manifest_file(cls, manifest_file: Union[str, Path], **kwargs):
        ...
        if feats_type == "mediapipe_keypoints":
            feats_type = "mediapipe" # TODO: change this to be the same as in TD
    ...
    def postprocess(self, pose):
        if SignFeatsType[self.feats_type] in [SignFeatsType.mediapipe, SignFeatsType.openpose]:
            if len(pose.header.components) == 5: #This means that we have all the keypoints
                if self.bodyparts == ["LEFT_HAND_LANDMARKS", "RIGHT_HAND_LANDMARKS"]: #This usually means to select the hands.
                    pose = pose.get_components(
                        self.bodyparts
                    )
                    if self.feat_dims == [0,1]:
                        for i in range(len(pose.header.components)):
                            pose.header.components[i].format = "XYC"
                        pose.body.data = pose.body.data[:, :, :, :2] #We are removing the "depth" dimension
                else:
                    ...
                    ) # this is to select the upper body and only some of the keypoints.
            elif len(pose.header.components) == 2: #This means that we only have the hands.
                # TODO: implement this for the egosign dataset
                # but we already have the hands, so no need to select
                if self.feat_dims == [0,1]:
                    for key in pose.header.components:
                        pose.header.components[key].format = "XYC"
                    pose.body.data = pose.body.data[:, :, :2] #We are removing the "depth" dimension
            
            
            if self.normalization == NormType.body:
                if "POSE_LANDMARKS" in [component.name for component in pose.header.components]:
            ...
            elif self.normalization == NormType.layer_norm: # Check why here we only have this string ? 
                #I think this would be equivalent to kp_wise, or global_xys ?
                num_components = len(pose.header.components)
                # TODO: this only works when we are selecting the two hands
                seq_len, _, n_feats, n_dims = pose.body.data.shape
                ...
                pose_split = pose.body.data.transpose(3, 0, 1, 2) # to have the x and y dimensions in the first axis
                with torch.no_grad():
                    pose_norm_split = F.layer_norm(torch.from_numpy(pose_split), pose_split.shape[1:]) #to do layer norm independently for each dimension
                pose.body.data = np.ma.MaskedArray(pose_norm_split.numpy().transpose(1, 2, 3, 0), pose.body.data.mask)
            elif self.normalization == NormType.center_and_scale.name:
                pose = self.center_and_scale(pose)
            else:
                pass
            
            if self.data_augmentation: 
                import pdb; pdb.set_trace()
                # TODO: here figure out if this works after we have normalized, I guess it does!
                pose = pose.augment2d()
